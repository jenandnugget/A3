/******************************************************************************
 * Project by Negin Sauermann & Jen Cooper 
 * Purpose: Association Rule Mining
 * Referenced + StackOverflow (as linked): Apriori Algorithm by Nathan Magnus created in 2009, free to implement with proper reference.
...
//for file input, vector and tokenizing:
...
public class Apriori {
	...
	Scanner s = new Scanner(System.in); //userInput
    ...
    //No argument constructor to house impliedData, resultingData, supportCalculate, and confidenceRate
	...
	public void getInput(){
		...
		inputFile = s.nextLine(); //set user input to inputFile for tokenizing
	
		System.out.println("Select the minium support rate, values range from 0.00-1.00:");
		minRate = s.nextDouble();
	...
	public void loadData() throws IOException{
		...
		BufferedReader reader = null;
			...
			reader = new BufferedReader(new FileReader(inputFile)); //from user input
			...
			while ((currentLine = reader.readLine()) != null) { 
				...
				//src: http://stackoverflow.com/questions/3034757/how-to-split-a-string-array
				String[] words = currentLine.split("\\s+"); //removing spaces
				for(int i = 0; i < words.length;i++){
	 ...
	 *****************************************************************************/
	public void generateTheCandidates()
	{
		...
		for (int j = 0; j<data.size(); j++)
		{
			for (int k = j+1; k <data.size(); k++)
			{
				...
				token1 = new StringTokenizer(data.get(j));
				token2 = new StringTokenizer(data.get(k));
				
				//if there are still items to go, reinitalize the candidates
				for (int x = 0; x < currentItemS - 2; x++)
				{
					firstCandidate = firstCandidate + " " + token1.nextToken();
					secondCandidate = secondCandidate + " " + token2.nextToken();
				}//end x for
	 ...
	 *****************************************************************************/
	public void generateFrequentItemS()
			{
